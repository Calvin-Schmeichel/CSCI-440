    ...
    def move(self, board):
        ...
        for dx, dy in directions:
            ...
            if board.grid[new_x][new_y] == '*':  # Move only if the spot is empty
                if board.grid[self.x][self.y] == '@':
                    # Eat the food and remove it from the board and food list
                    self.eat_food(board, self.x, self.y)
                board.grid[self.x][self.y] = '*'
                self.x, self.y = new_x, new_y
                break
        board.update_position(self)

    def eat_food(self, board, x, y):
        self.health += 50
        board.grid[x][y] = '*'
        board.food.remove((x, y))
