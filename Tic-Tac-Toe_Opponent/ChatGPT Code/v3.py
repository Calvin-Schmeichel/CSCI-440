...
def on_click(row, col):
    ...
    if board[row][col] is None and winner is None:
        ...
        if winner:
            ...
            new_game_button.grid(row=4, column=0, columnspan=3)  # Show New Game button
        elif player == 'O':
            ai_move()
...
def reset_game():
    ...
    new_game_button.grid_remove()  # Hide New Game button
...
new_game_button = tk.Button(app, text='New Game', command=reset_game)
new_game_button.grid(row=4, column=0, columnspan=3)
new_game_button.grid_remove()  # Initially hide the New Game button
